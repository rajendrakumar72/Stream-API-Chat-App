package com.mrkumar.streamchatapp.ui.channel
...
import com.mrkumar.streamchatapp.databinding.FragmentChannelBinding
...
class ChannelFragment:Fragment() {
    ...
    private fun setupUser() {
        if (client.getCurrentUser() == null) {
            user = if (args.chatUser.firstName.contains("MRK72")) {
                User(
                    id = args.chatUser.username,
                    extraData = mutableMapOf(
                        "name" to args.chatUser.firstName,
                        "county" to "India",
                        "image" to "https://avatars.githubusercontent.com/u/73107869?v=4"
                    )
                )
            } else {
                User(
